...
from and_platform.models import db, ChallengeReleases, CheckerQueues, CheckerVerdict, Servers, Teams
from flask import Blueprint, jsonify
from and_platform.core.config import get_config
from sqlalchemy.sql import func

public_server_blueprint = Blueprint("public_server_blueprint", __name__, url_prefix="/servers")

@public_server_blueprint.get("/")
@cache.cached()
def get_all_servers():
    rows = db.session.query(Servers, Teams).join(Teams, Teams.server_id == Servers.id).all()
    response = {}
    for row in rows:
        server, team = row
        response[team.id] = server.host
...
@public_server_blueprint.get("/status")
